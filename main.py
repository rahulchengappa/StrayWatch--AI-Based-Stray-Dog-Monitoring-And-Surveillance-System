...
# ========================================================
# DETECTION DATA ENDPOINT (SURVEILLANCE PAGE)
# ========================================================
...
# ========================================================
# DISCONNECT DETECTION (WEBRTC AUTOSTOP)
# ========================================================
...
# ========================================================
# STREAMING FUNCTION WITH LOGGING SUPPORT
# ========================================================
                ...
                for box in r.boxes:
                    ...
                    if conf > 0.50:
                        ...
                        if roi.size > 0:
                            ...
                            # Encode roi thumbnail
                            _, buf = cv2.imencode(".jpg", roi)
                            thumbnail_b64 = base64.b64encode(buf).decode()
                        ...
                        cv2.putText(frame, emotion, (x1,y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0),2)
    ...
    <body>
        <script>
            navigator.geolocation.getCurrentPosition(pos => {
                window.location.href = "/camera_stream?lat=" + pos.coords.latitude + "&lon=" + pos.coords.longitude;
            });
        </script>
...
def camera_stream(lat: float, lon: float):
    ...
    <body style="margin:0;background:black;">
        <img src="/video_feed?lat={lat}&lon={lon}" style="width:100vw;height:100vh;object-fit:contain">
        <button onclick="window.location.href='/'">Back Home</button>
    </body>
    </html>
    """
# ========================================================
# SURVEILLANCE PAGE (Leaflet Map)
# ========================================================
@app.get("/surveillance", response_class=HTMLResponse)
def surveillance():
    with open("surveillance.html") as f:
        return f.read()
...
def home():
    with open("homepage.html") as f:
        return f.read()
# ========================================================
# ANALYZE UPLOAD IMAGE/VIDEO ENDPOINT (with logging)
# ========================================================
@app.post("/analyze")
async def analyze(file: UploadFile = File(...)):
    try:
        ...
        # Resize huge images (YOLO becomes unstable otherwise)
        h, w = frame.shape[:2]
        if max(h, w) > 1200:
            scale = 1200 / max(h, w)
            frame = cv2.resize(frame, (int(w * scale), int(h * scale)))

        results = yolo_model(frame, classes=[16])  # detect only dogs
        detections = []

        # DEFAULT LOCATION (0,0) unless you add geolocation in upload page
        lat, lon = 0.0, 0.0
